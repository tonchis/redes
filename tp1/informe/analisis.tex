...
\indent Vemos que resulta interesante la gran cantidad de veces que se solicita la IP 192.168.0.193. Para saber más sobre lo que podría estar sucediendo con ese host, revisamos el resto de la captura pero encontramos que no había ningún paquete con esa dirección origen ni destino.\\
...
\indent Más en detalle, los datos de los nodos que consideramos distinguidos son:\\
\begin{center}
\begin{tabular}{c c}
\begin{tabular}{ | c | c  | }
	\hline
	Entropía origen & 3.44 \\
	\hline
	IP                      & Información \\
	\hline
	192.168.0.1     & 1.41 \\
...
Con estos nuevos datos, recordemos el gráfico con los nodos que comparten más de mil mensajes:\newline

\includegraphics[scale=0.30]{graphics/t-work-all-1000w.png}

Vemos cómo los dos nodos que menos \textit{información} nos proveen son los que más \textit{request ARP} intercambian, 192.168.0.1 busca a 192.168.0.192 21301 veces. También se confirma que el router es uno de los nodos más requeridos de la red.\newline
...
\indent \indent Analizando las direcciones IPs que más pedidos hicieron, se obtuvo el siguiente histograma, que representa la información calculada por IP.
...
\includegraphics[scale=0.5,clip=true,trim=100 0 0 0]{graphics/facultad_mediana_dst.png}
